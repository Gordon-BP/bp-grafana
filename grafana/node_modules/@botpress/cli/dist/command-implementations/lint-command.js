...
class LintCommand extends import_project_command.ProjectCommand {
  ...
  _dereferenceActionDefinition = (actionDef) => {
    ...
    actionDef.input.schema = actionDef.input.schema.dereference(inputRefSchemas);
    actionDef.output.schema = actionDef.output.schema.dereference(outputRefSchemas);
    return actionDef;
  };
  _dereferenceEventDefinition = (eventDef) => {
    const refs = eventDef.schema.getReferences();
    const refSchemas = Object.fromEntries(refs.map((ref) => [ref, this._replaceRef(ref)]));
    eventDef.schema = eventDef.schema.dereference(refSchemas);
    return eventDef;
  };
  _replaceRef = (refUri) => import_sdk.z.object({}).title(refUri).describe(refUri);
}
