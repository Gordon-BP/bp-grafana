...
var UpstreamProviderError = class extends BaseApiError {
  constructor(message, error, id) {
    super(502, "The upstream provider returned an error or is currently unavailable.", "UpstreamProvider", message, error, id);
  }
};
var errorTypes = {
  ...
  UpstreamProvider: UpstreamProviderError
};
var errorFrom = (err) => {
  if (isApiError(err)) {
    return err;
...
__publicField(WatchDog, "init", (ms) => {
...
var checkPayloadForError = (response) => {
  if (!("code" in response)) {
    return response;
  }
  const { code } = response;
...
var wrapAutoClient = (client) => new Proxy(client, {
  get(target, prop) {
...
var _Client = class {
  constructor(props) {
    ...
    this._auto = wrapAutoClient(new Client(axiosClient));
  }
  _auto;
  get apiVersion() {
    return apiVersion;
